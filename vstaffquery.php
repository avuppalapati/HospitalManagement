...
if($staff == "Nurses"){
    ...
    $query = "SELECT doc.drname, doc.sex, doc.dob, dep.dname FROM doctors doc, departments dep, works_in w 
    WHERE doc.dssn = w.dssn_works_in AND dep.did = w.did_works_in";    
    if (!empty($sex)){
        $query .= " AND sex = '$sex'";
    }
    if(!empty($dob)){
